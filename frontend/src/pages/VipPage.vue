...
import { createVipPaymentSession, completeMockVipPayment } from '@/api/paymentController'
...
const handlePurchase = async () => {
  ...
  try {
    ...
    Modal.success({
      title: '支付成功！',
      content: '恭喜您成为永久会员，已解锁全部高级功能！（Mock）',
      okText: '开始创作',
      onOk: () => {
        router.push('/create')
      },
    ...
    const msg = error instanceof Error ? error.message : '创建支付失败，请稍后重试'
    message.error(msg)
  } finally {
